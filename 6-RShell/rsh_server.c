...
int process_cli_requests(int svr_socket){
    ...
    while(1){
        ...
        if ( (exec_client_requests(cli_socket)) == BI_CMD_STOP_SVR) {
...
int exec_client_requests(int cli_socket) {
    ...
    while(1) {
        ...
        // build up a cmd_list
        rc = buildCommandListT(io_buff, &cmd_list);
        if (rc < 0) {
            return rc;
        }
        ...
        //     return rc;
        // }
        // was doing this and not getting the output i wanted so i'm not gonna use it for now
        ...
        else if (last_rc == BI_CMD_STOP_SVR) {
...
int rsh_execute_pipeline(int cli_sock, command_list_t *clist) {
    ...
    for (int i = 0; i < clist->num; i++) {
        ...
        if (pids[i] == 0) {  // Child process
            // Set up input pipe for all processes
            if (i > 0) {
            ...
            //built-ins here
            exit_code = rsh_built_in_cmd(&(clist->commands[i]));
            switch (exit_code) {
                case BI_CMD_EXIT:
                    exit(EXIT_SC);
                case BI_CMD_STOP_SVR:
                    exit(BI_CMD_STOP_SVR);
                case BI_NOT_BI:
                    break;
                default:
                    exit(BI_EXECUTED);
